...
;
;
lastblk        :=    $D014           ; flag for last block
blkno          :=    $D015           ; block number
errcnt         :=    $D016           ; error counter 10 is the limit
bflag          :=    $D017           ; block flag

crc            :=    $D018           ; CRC lo byte  (two byte variable)
crch           :=    $D019           ; CRC hi byte

ptr            :=    zp_bas_tmpStr_1
ptrh           :=    zp_bas_tmpStr_1+1
; ptr            :=    $1a           ; data pointer (two byte variable)
; ptrh           :=    $1b           ;   "    "

eofp           :=    $D01c           ; end of file address pointer (2 bytes)
eofph          :=    $D01d           ;  "    "    "    "

retry          :=    $D01e           ; retry counter
retry2         :=    $D01f           ; 2nd counter
...
;
; $D000 = Screen RAM
Rbuff          :=    $D100             ; temp 132 byte receive buffer
...
;
CRCLO          :=     $D200             ; Two 256-byte tables for quick lookup
CRCHI          :=     $D300             ; (should be page-aligned for speed)
;
;
;
...
;
        .org $F800
;        *=     $F800           ; Start of program (adjust to your needs)
;
...
;
ext_mon:
; This is a patch, we need to do what the caller USED to do
              cmp    #$4C           ; A-Reg = 'L'
              beq    ext_SetLoadSaveFlag
;
              jsr    inputChar      ; Check the KBD and/or TAPE port
              cmp    #'X'           ; CMD = X)Modem
              beq    ext_xmodem     ;
              cmp    #'B'           ; CMD = B)asic pgm loader
              beq    ext_basic      ;
              rts                   ; No valid keys!
;
; We arrive in ext_mon
ext_SetLoadSaveFlag:
              pla                   ; Stack Shenannigans
              pla                   ; Stack Shenannigans
              jmp    mon_SetLoadSaveFlag
...
ext_basic_load:
                                    ; Read bytes from KBD/TAPE & stuff into buffer
                                    ; If CHAR = NULL then send XOFF back to source
                                    ; Call BASIC ROM rtyn to process line
                                    ; Once we are back loop back for another line
                                    ; Unless we are no longer loading <SPC> KEYB key hit?
              jmp     zp_bas_JmpWarm; Call Basic WarmStart?
ext_xmodem:
              setCurs 5,4
              jsr    clrLine
              jsr    crtXModem
...
;
              jsr    aciaWelcome1    ; send prompt and info
              jsr    aciaWelcome2    ; send prompt and info
;
              lda    #$00           ;
...
PrtAbort:     jsr    Flush          ; yes, too many errors, flush buffer,
              jmp    crtError       ; print error msg and exit
              rts
Done:         Jmp    crtSuccess     ; All Done..Print msg and exit
              rts
...
GetBlk2:      sta    Rbuff,x        ; good char, save it in the rcv buffer
              ...
              jsr    crtError       ; Unexpected block number - abort
...
@Loop:          jsr     aciaGetR        ; get chr from serial port, don't wait
...
@Loop:          jsr     GetByte         ; read the port
...
;
                makelabel       "foo", "bar"
                .word           foo_bar          ; Valid label
                jmp     foo_bar
;
                mkStrPrt crt, strBasic
;
                jmp     crt_strBasic
;
aciaStrOut:
...
; Welcome1
aciaWelcome1:
@offset = ( strWelcome1 - stringTable )&$FF
                ldx     #@offset
                jsr     aciaStrOut
                rts
; Welcome2
aciaWelcome2:
@offset = ( strWelcome2 - stringTable )&$FF
                ldx     #@offset
                jsr     aciaStrOut
                rts
;
...
;
crtXModem:
@offset = ( strXModem - stringTable )&$FF
                ldx     #@offset
                jsr     crtStrOut
                rts
;
;
crtLoad:
@offset = ( strLoad - stringTable )&$FF
                ldx     #@offset
                jsr     crtStrOut
                rts
;
crtSave:
@offset = ( strSave - stringTable )&$FF
                ldx     #@offset
                jsr     crtStrOut
                rts
;
...
;
crtExamine:
@offset = ( strExamine - stringTable )&$FF
                ldx     #@offset
                jsr     crtStrOut
                rts
;
crtError:
@offset = ( strError - stringTable )&$FF
                ldx     #@offset
                jsr     crtStrOut
                rts
;
crtSuccess:
@offset = ( strSuccess - stringTable )&$FF
                ldx     #@offset
                jsr     crtStrOut
                rts
;
...
;
        rts
;
stringTable:    ; MAX 5 display chars + one NULL
strStart:       .asciiz   "Start"
strEnd:         .asciiz   "End"
strLoad:        .asciiz   "Load"
strSave:        .asciiz   "Save"
strAddr:        .asciiz   "Addr"
strSuccess:     .asciiz   "Successful"
strError:       .asciiz   "Error!"
strTransfer:    .asciiz   "Transfer "
strWelcome1:    .asciiz   "Begin XMODEM/CRC xfer"
strWelcome2:    .asciiz   "Press <Esc> to abort"
strMove:        .asciiz   "Move"
strExamine:     .asciiz   "Examine"
strBasic:       .asciiz   "Basic"
strXModem:      .asciiz   "XModem"
